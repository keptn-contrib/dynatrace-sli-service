...
With Dynatrace Version 207 Dynatrace introduced native support for SLO monitoring. The *dynatrace-sli-service* is able to query these SLO definitions by referencing them by SLO-ID. Here is such an SLO as seen in a dashboard:
...
The *dynatrace-sli-service* will also define a key SLO for `problem_open` with a default pass criteria of `<=0` meaning the evaluation will only succeed if the problem is closed. The following is an excerpt of that SLO definition:
```yaml
objectives:
- sli: problem_open
  pass:
